        ...
        <div class="hidden lg:flex flex-col space-y-8">
            ...
            <div class="space-y-4 max-w-md">
                <div class="flex items-start gap-4 p-4 glass-panel border border-[#D8C9B8] polygon-card bg-[#F8F5EF]">
                    ...
                    <div>
                        <span class="text-sm font-bold text-[#29241F] block">{{ __('Track Orders Like A Pro') }}</span>
                        <span class="text-xs text-gray-600 font-medium">{{ __('See every delivery status from fragrance atelier to your doorstep.') }}</span>
                    </div>
                </div>
                <div class="flex items-start gap-4 p-4 glass-panel border border-[#D8C9B8] polygon-card bg-[#F8F5EF]">
                    ...
                    <div>
                        <span class="text-sm font-bold text-[#29241F] block">{{ __('Wishlist Your Dream Scents') }}</span>
                        <span class="text-xs text-gray-600 font-medium">{{ __('Curate and save your perfect collection across every device.') }}</span>
                    </div>
                </div>
                <div class="flex items-start gap-4 p-4 glass-panel border border-[#D8C9B8] polygon-card bg-[#F8F5EF]">
                    ...
                    <div>
                        <span class="text-sm font-bold text-[#29241F] block">{{ __('VIP Early Access Drops') }}</span>
                        <span class="text-xs text-gray-600 font-medium">{{ __('Members get exclusive 48-hour early access to limited collections.') }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div>
            <div class="glass-panel-gold p-8 sm:p-10 polygon-card border-2 border-[#A8895F]/40 shadow-2xl bg-[#F8F5EF]">
                <div class="mb-8">
                    ...
                    <h2 class="font-serif font-bold text-3xl text-[#29241F]">{{ __('Create Your Account') }}</h2>
                    <p class="text-sm text-gray-600 font-medium mt-2">
                        {{ __('Already a member?') }}
                        <a href="{{ route('login') }}" class="text-[#A8895F] font-bold underline hover:text-[#29241F] transition-colors ml-1">{{ __('Sign in here') }} →</a>
                    </p>
                </div>
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-4">
                    ...
                    <div>
                        <label for="phone" class="block text-[10px] font-extrabold text-[#A8895F] uppercase tracking-[0.25em] mb-2">
                            {{ __('Phone Number') }} <span class="text-gray-400 font-normal normal-case">{{ __('(Recommended for order updates)') }}</span>
                        </label>
                        <input
                            id="phone"
                            type="tel"
