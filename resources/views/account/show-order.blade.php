...
@section('title', 'Order '.$order->order_number.' | Sozie Collection')

@section('content')
@include('account._sidebar_layout', [
    'account_title' => 'Order #'.$order->order_number,
    'account_subtitle' => 'Complete order details, fragrance line-up, and real-time delivery status for this Sozie Collection order.'
])
@endsection
        ...
        <div class="lg:col-span-2 space-y-5">
            <div class="glass-panel-gold border-2 border-[#A8895F]/25 polygon-card bg-[#F8F5EF] p-6 shadow-lg">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        <span class="text-[10px] font-extrabold text-[#A8895F] uppercase tracking-[0.3em] block mb-0.5">Order Timeline</span>
                        <h3 class="font-serif font-bold text-xl text-[#29241F]">Live Delivery Progress</h3>
                    </div>
                </div>
                ...
                <div class="relative pl-8 pb-1">
                    <ol class="relative border-l-2 border-[#D8C9B8] ml-2 space-y-7">
                        @php
                            $steps = [
                                ['key' => 'placed', 'icon' => 'clipboard-list', 'label' => 'Order Placed', 'desc' => 'Your fragrance selections received & verified at the Sozie atelier.'],
                                ['key' => 'processing', 'icon' => 'flask-conical', 'label' => 'Processing', 'desc' => 'Each bottle is hand-prepared, wrapped, and quality inspected by our team.'],
                                ['key' => 'shipped', 'icon' => 'truck', 'label' => 'Shipped', 'desc' => 'Package in transit. Signature delivery scheduled to your saved address.'],
                                ['key' => 'delivered', 'icon' => 'gift', 'label' => 'Delivered', 'desc' => 'Delivered! Enjoy your new signature scent. Welcome to Sozie Collection.'],
                            ];
                        ...
                        <li class="relative">
                            ...
                            <h5 class="font-extrabold text-sm text-rose-700 uppercase tracking-[0.15em]">Order Cancelled</h5>
                            <p class="text-[11px] text-gray-600 font-medium mt-1 leading-relaxed">This order was cancelled. Please contact Sozie Collection support for any questions.</p>
                        </li>
                        @else
                        ...
                        <li class="relative">
                            ...
                            <h5 class="font-extrabold text-sm uppercase tracking-[0.15em] {{ $isDone ? 'text-[#29241F]' : 'text-gray-400' }}">
                                {{ $step['label'] }}
                                @if($isCurrent)
                                <span class="ml-2 inline-block px-2 py-0.5 bg-[#A8895F]/20 text-[#A8895F] text-[9px] uppercase tracking-wider rounded animate-pulse font-extrabold">Now</span>
                                @endif
                            </h5>
            ...
            <div class="glass-panel border border-[#D8C9B8] polygon-card bg-[#F8F5EF] p-6 shadow-md">
                <div class="flex items-center justify-between mb-5">
                    <div>
                        <span class="text-[10px] font-extrabold text-[#A8895F] uppercase tracking-[0.3em] block mb-1">Fragrances in This Order</span>
                        <h3 class="font-serif font-bold text-xl text-[#29241F]">Items Summary</h3>
                    </div>
                    <span class="text-[11px] font-extrabold text-gray-500 uppercase tracking-wider">
                        {{ $order->items?->count() ?? 0 }} item(s)
                    </span>
                </div>

                <div class="space-y-3">
                    @if($order->items?->isEmpty())
                    <div class="text-center py-12 text-gray-500 text-xs font-bold">No line items found.</div>
                    @else
                    ...
                    <div class="flex gap-4 p-3 bg-white border border-[#D8C9B8] polygon-card hover:border-[#A8895F]/50 transition-colors">
                        ...
                        <div class="flex-1 min-w-0 flex flex-col justify-between">
                            ...
                            <div class="flex items-center justify-between pt-2">
                                <span class="text-[10px] font-bold text-gray-600 uppercase tracking-wider">Qty: {{ $item->quantity }}</span>
        ...
        <aside class="space-y-5">
            <div class="glass-panel-gold border-2 border-[#A8895F]/25 polygon-card bg-[#F8F5EF] p-5 shadow-xl">
                ...
                <dl class="space-y-3 text-[11px] font-bold">
                    ...
                    <div class="flex justify-between text-emerald-700">
                        <dt>Discount {{ !empty($order->discount_code) ? "($order->discount_code)" : '' }}</dt>
                    ...
                    <div class="flex justify-between items-center pt-3 mt-3 border-t border-[#D8C9B8] text-[#29241F]">
                        <dt class="text-[10px] uppercase tracking-[0.2em] font-extrabold text-[#A8895F]">Total Paid</dt>
                ...
                <div class="mt-4 pt-4 border-t border-[#D8C9B8] space-y-2">
                    <div class="flex items-center justify-between text-[11px]">
                        <span class="text-gray-500 font-bold uppercase tracking-wider">Payment</span>
                        <span class="font-extrabold text-[#29241F]">{{ $order->payment_method ?? 'Cash on Delivery' }}</span>
                    </div>
                    <div class="flex items-center justify-between text-[11px]">
                        <span class="text-gray-500 font-bold uppercase tracking-wider">Status</span>
                        <span class="font-extrabold text-[#29241F]">{{ $order->payment_status ?? 'Pending' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-panel border border-[#D8C9B8] polygon-card bg-[#F8F5EF] p-5 shadow-md">
                ...
                <address class="not-italic text-xs text-gray-700 font-medium leading-relaxed space-y-1.5">
                    ...
                    @endif
                    <p class="pt-1">
                        {{ $order->shipping_address ?? $order->address_street ?? 'No street address recorded' }}
                    </p>
                    <p>{{ $order->shipping_city ?? $order->address_city ?? 'City' }}</p>
                </address>
            </div>
