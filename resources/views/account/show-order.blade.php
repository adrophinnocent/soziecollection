...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        <a href="{{ route('account.orders') }}"
           class="inline-flex items-center gap-2 text-[11px] font-extrabold uppercase tracking-[0.2em] text-[#A8895F] hover:text-[#F8F5EF] transition-colors">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div class="lg:col-span-2 space-y-5">
            <div class="glass-panel-gold border-2 border-[#A8895F]/25 polygon-card bg-[#17130F] p-6 shadow-lg">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <h3 class="font-serif font-bold text-xl text-[#EDE5D8]">{{ __('Live Delivery Progress') }}</h3>
                    </div>
                </div>

                <div class="relative pl-8 pb-1">
                    <ol class="relative border-l-2 border-[#322B23] ml-2 space-y-7">
                        @php
                            $steps = [
                        ...
                        @if($isCancelled)
                        <li class="relative">
                            <span class="absolute -left-[41px] flex h-9 w-9 items-center justify-center rounded-full bg-rose-600 border-4 border-[#17130F] shadow-md">
                                <i data-lucide="ban" class="w-4 h-4 text-white"></i>
                            </span>
                            <h5 class="font-extrabold text-sm text-rose-300 uppercase tracking-[0.15em]">{{ __('Order Cancelled') }}</h5>
                            <p class="text-[11px] text-[#B5A897] font-medium mt-1 leading-relaxed">{{ __('This order was cancelled. Please contact Sozie Collection support for any questions.') }}</p>
                        </li>
                        @else
                        ...
                        @endphp
                        <li class="relative">
                            <span class="absolute -left-[41px] flex h-9 w-9 items-center justify-center rounded-full border-4 border-[#17130F] shadow-md
                                {{ $isDone ? 'bg-[#A8895F] text-[#12100E]' : 'bg-[#0C0A09] text-[#A8895F] border-[#322B23]' }}">
                                <i data-lucide="{{ $step['icon'] }}" class="w-4 h-4"></i>
                            </span>
                            <h5 class="font-extrabold text-sm uppercase tracking-[0.15em] {{ $isDone ? 'text-[#EDE5D8]' : 'text-[#A89C8C]' }}">
                                {{ $step['label'] }}
                                @if($isCurrent)
                            ...
                            <p class="text-[11px] {{ $isDone ? 'text-[#B5A897]' : 'text-[#A89C8C]' }} font-medium mt-1 leading-relaxed">{{ $step['desc'] }}</p>
                        </li>
                        @endforeach
                        @endif
                    </ol>
            ...
            <div class="glass-panel border border-[#322B23] polygon-card bg-[#17130F] p-6 shadow-md">
                <div class="flex items-center justify-between mb-5">
                    <div>
                        ...
                        <h3 class="font-serif font-bold text-xl text-[#EDE5D8]">{{ __('Items Summary') }}</h3>
                    </div>
                    <span class="text-[11px] font-extrabold text-[#A89C8C] uppercase tracking-wider">
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-center py-12 text-[#A89C8C] text-xs font-bold">{{ __('No line items found.') }}</div>
                    @else
                    @foreach($order->items as $item)
                    <div class="flex gap-4 p-3 bg-[#17130F] border border-[#322B23] polygon-card hover:border-[#A8895F]/50 transition-colors">
                        <div class="w-20 h-20 flex-shrink-0 bg-[#0C0A09] polygon-card border border-[#322B23] overflow-hidden">
                            ...
                            @else
                            <div class="w-full h-full flex items-center justify-center text-[#A8895F]">
                        ...
                        <div class="flex-1 min-w-0 flex flex-col justify-between">
                            <div>
                                <p class="text-xs font-extrabold text-[#EDE5D8] leading-snug mb-1">{{ $item->name }}</p>
                                <div class="flex flex-wrap gap-2 text-[10px] font-bold">
                                    @if(!empty($item->size))
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-[#0C0A09] text-[#EDE5D8] rounded uppercase tracking-wider polygon-badge">
                                        {{ $item->size }}
                                    </span>
                                    @endif
                                    <span class="text-[#A89C8C]">SKU {{ $item->sku ?? ($item->product?->sku ?? '—') }}</span>
                                </div>
                            </div>
                            <div class="flex items-center justify-between pt-2">
                                <span class="text-[10px] font-bold text-[#B5A897] uppercase tracking-wider">{{ __('Qty:') }} {{ $item->quantity }}</span>
        ...
        <aside class="space-y-5">
            <div class="glass-panel-gold border-2 border-[#A8895F]/25 polygon-card bg-[#17130F] p-5 shadow-xl">
                <h4 class="font-serif font-bold text-lg text-[#EDE5D8] mb-4 pb-3 border-b border-[#322B23]">
                    {{ __('Payment & Totals') }}
                </h4>
                <dl class="space-y-3 text-[11px] font-bold">
                    ...
                    @endif
                    <div class="flex justify-between items-center pt-3 mt-3 border-t border-[#322B23] text-[#EDE5D8]">
                        <dt class="text-[10px] uppercase tracking-[0.2em] font-extrabold text-[#A8895F]">{{ __('Total Paid') }}</dt>
                        <dd class="font-serif font-bold text-2xl text-[#EDE5D8]">
                            TZS {{ number_format($order->total_amount ?? 0, 0) }}
                        </dd>
                    </div>
                </dl>
                <div class="mt-4 pt-4 border-t border-[#322B23] space-y-2">
                    <div class="flex items-center justify-between text-[11px]">
                        <span class="text-[#A89C8C] font-bold uppercase tracking-wider">{{ __('Payment') }}</span>
                        <span class="font-extrabold text-[#EDE5D8]">{{ $order->payment_method ?? __('Cash on Delivery') }}</span>
                    </div>
                    <div class="flex items-center justify-between text-[11px]">
                        <span class="text-[#A89C8C] font-bold uppercase tracking-wider">{{ __('Status') }}</span>
                        <span class="font-extrabold text-[#EDE5D8]">{{ $order->payment_status ?? __('Pending') }}</span>
                    </div>
                </div>
            </div>

            <div class="glass-panel border border-[#322B23] polygon-card bg-[#17130F] p-5 shadow-md">
                <h4 class="font-serif font-bold text-base text-[#EDE5D8] mb-4 pb-3 border-b border-[#322B23] flex items-center gap-2">
                    <i data-lucide="map-pin" class="w-4 h-4 text-[#A8895F]"></i>
                    {{ __('Delivery Address') }}
                </h4>
                <address class="not-italic text-xs text-[#B5A897] font-medium leading-relaxed space-y-1.5">
                    <p class="font-extrabold text-[#EDE5D8] text-sm">
            ...
            <div class="flex flex-col gap-2.5">
                <a href="{{ route('shop.index') }}"
                   class="px-5 py-3 bg-[#A8895F] text-[#12100E] text-[10px] font-extrabold uppercase tracking-[0.25em] polygon-btn hover:bg-[#12100E] hover:text-[#F8F5EF] transition-all text-center shadow-md inline-flex items-center justify-center gap-2">
