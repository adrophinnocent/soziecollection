...
<div class="glass-panel border border-[#D8C9B8] polygon-card bg-[#F8F5EF] p-5 shadow-md mb-5 flex flex-wrap items-center justify-between gap-4">
...
<div class="glass-panel border border-[#D8C9B8] polygon-card bg-[#F8F5EF] p-6 shadow-md">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        <div class="navy-card p-4 polygon-card flex flex-col bg-white border border-[#D8C9B8] relative overflow-hidden group hover:shadow-xl hover:border-[#A8895F]/50 transition-all">
            <div class="relative aspect-square bg-[#EDE5D8] polygon-card overflow-hidden mb-4 border border-[#D8C9B8]">
                ...
                <form action="{{ route('wishlist.remove') }}" method="POST" class="absolute top-2 right-2">
                    ...
                    <button type="submit"
                            class="w-8 h-8 bg-white/90 backdrop-blur rounded-full flex items-center justify-center shadow-md text-rose-600 hover:text-rose-700 hover:bg-white transition-all border border-rose-100">
            ...
            <div class="flex-1 flex flex-col min-h-0">
                <div class="mb-2">
                    ...
                    <a href="{{ route('shop.show', $product->slug) }}"
                       class="font-serif font-bold text-base text-[#29241F] leading-tight hover:text-[#A8895F] transition-colors line-clamp-2">
                        {{ $product->name }}
                    </a>
                    @else
                    ...
                    <p class="text-[10px] text-gray-500 font-medium line-clamp-1 mt-1">{{ $product->tagline }}</p>
                    @endif
                </div>
                <div class="mt-auto space-y-3 pt-3 border-t border-[#D8C9B8]/60">
