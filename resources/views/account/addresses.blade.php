...
<div class="flex flex-wrap items-center justify-between gap-4 mb-5">
    <div class="flex items-center gap-3 text-[11px] font-bold text-[#B5A897]">
...
<div class="glass-panel border border-[#322B23] polygon-card bg-[#17130F] p-6 shadow-md">
    ...
    <div class="py-20 text-center">
        <div class="w-20 h-20 mx-auto mb-6 rounded-full bg-[#A8895F]/10 border border-[#A8895F]/30 flex items-center justify-center">
            <i data-lucide="map-pinned" class="w-10 h-10 text-[#A8895F]"></i>
        </div>
        <h3 class="font-serif font-bold text-2xl text-[#EDE5D8] mb-2">{{ __('No saved addresses yet') }}</h3>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="navy-card p-5 polygon-card bg-[#17130F] border border-[#322B23] relative overflow-hidden hover:shadow-md transition-shadow
            {{ $address->is_default ? 'ring-2 ring-[#A8895F] ring-offset-2 ring-offset-[#17130F]' : '' }}">
            @if($address->is_default)
            <span class="absolute top-4 right-4 inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-[#A8895F] text-[#12100E] text-[9px] uppercase tracking-[0.2em] font-extrabold shadow">
            ...
            <div class="flex items-start gap-3 mb-3">
                ...
                <div class="min-w-0 flex-1">
                    <h4 class="font-serif font-bold text-lg text-[#EDE5D8] leading-tight mb-1 pr-16">{{ $address->label }}</h4>
                    <p class="text-xs font-extrabold text-[#EDE5D8] leading-snug mb-0.5">{{ $address->full_name }}</p>
                    <p class="text-[11px] text-[#B5A897] font-bold flex items-center gap-1.5">
            ...
            <address class="not-italic text-xs text-[#B5A897] font-medium leading-relaxed bg-[#100E0C]/60 border border-[#322B23] p-3 polygon-card mb-4 min-h-[60px]">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('account.addresses.edit', $address->id) }}"
                   class="px-3 py-1.5 bg-[#17130F] border border-[#A8895F]/50 text-[#EDE5D8] text-[10px] font-extrabold uppercase tracking-[0.2em] polygon-btn hover:bg-[#221D19] transition-all inline-flex items-center gap-1.5">
                ...
                <form action="{{ route('account.addresses.default', $address->id) }}" method="POST" class="inline-block">
                    ...
                    @method('PATCH')
                    <button type="submit"
                            class="px-3 py-1.5 bg-[#0C0A09] border border-[#322B23] text-[#EDE5D8] text-[10px] font-extrabold uppercase tracking-[0.2em] polygon-btn hover:bg-[#2C2620] transition-all inline-flex items-center gap-1.5">
                ...
                <form action="{{ route('account.addresses.destroy', $address->id) }}" method="POST" class="inline-block ml-auto"
                    ...
                    @method('DELETE')
                    <button type="submit"
                            class="px-3 py-1.5 bg-[#17130F] border border-[#7F1D3A] text-rose-300 text-[10px] font-extrabold uppercase tracking-[0.2em] polygon-btn hover:bg-[#2A1215] transition-all inline-flex items-center gap-1.5">
