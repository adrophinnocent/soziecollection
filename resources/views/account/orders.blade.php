...
@section('title', 'My Orders | Sozie Collection')

@section('content')
@include('account._sidebar_layout', [
    'account_title' => 'My Orders',
    'account_subtitle' => 'Every order with Sozie Collection. Track delivery status, view invoices, and reorder your favorite fragrances with a single click.'
])
@endsection
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            <form action="{{ route('orders.track.submit') }}" method="POST" class="flex flex-wrap items-stretch gap-2">
                @csrf
                <input type="text" name="order_number" placeholder="Find by order number (e.g. SOZ-20260924-001)"
...
<div class="glass-panel border border-[#D8C9B8] polygon-card bg-[#F8F5EF] p-6 shadow-md">
    ...
    <div class="py-20 text-center">
        ...
        <h3 class="font-serif font-bold text-2xl text-[#29241F] mb-2">Your fragrance journey is just beginning</h3>
        <p class="text-sm text-gray-500 font-medium max-w-md mx-auto mb-7">
            Once you place an order, it will appear here with real-time tracking from atelier prep through final delivery to your doorstep.
        </p>
        <a href="{{ route('shop.index') }}"
        ...
        <div class="border border-[#D8C9B8] bg-white polygon-card p-5 hover:shadow-md transition-shadow">
            <div class="flex flex-wrap items-start justify-between gap-4 mb-4 pb-4 border-b border-[#D8C9B8]/60">
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-4 mt-2 text-[11px] text-gray-600 font-bold">
                        ...
                        <span class="flex items-center gap-1.5">
                            ...
                            {{ $order->shipping_city ?? ($order->address_city ?? 'City not set') }}
                        </span>
                        <span class="flex items-center gap-1.5">
                ...
                <div class="text-right">
                    <span class="text-[10px] font-extrabold text-gray-500 uppercase tracking-[0.2em] block mb-1">Order Total</span>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 mb-4">
                ...
                <div class="aspect-square bg-[#29241F] text-[#F8F5EF] polygon-card border border-[#A8895F]/30 flex flex-col items-center justify-center gap-1">
                    ...
                    <span class="text-[9px] uppercase tracking-[0.2em] font-extrabold text-[#A8895F]">More</span>
                </div>
                @endif
            </div>
            @endif
