...
<div class="glass-panel border border-[#322B23] polygon-card bg-[#17130F] p-5 shadow-md mb-5">
        ...
        <div class="flex items-center gap-3">
            <form action="{{ route('orders.track.submit') }}" method="POST" class="flex flex-wrap items-stretch gap-2">
                ...
                <input type="text" name="order_number" placeholder="{{ __('Find by order number (e.g. SOZ-20260924-001)') }}"
                       class="min-w-[280px] bg-[#17130F] border border-[#322B23] text-[11px] text-[#EDE5D8] px-3 py-2 focus:outline-none focus:border-[#A8895F] focus:ring-2 focus:ring-[#A8895F]/50 font-bold placeholder:font-bold">
                <button type="submit"
                        class="px-4 py-2 bg-[#221D19] text-white text-[10px] font-extrabold uppercase tracking-[0.2em] polygon-btn hover:bg-[#A8895F] hover:text-[#12100E] transition-all">
                    {{ __('Search') }}
                </button>
            </form>
        </div>
        <a href="{{ route('shop.index') }}"
           class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-[#A8895F] text-[#12100E] text-[10px] font-extrabold uppercase tracking-[0.2em] polygon-btn hover:bg-[#12100E] hover:text-[#F8F5EF] shadow-md">
...
<div class="glass-panel border border-[#322B23] polygon-card bg-[#17130F] p-6 shadow-md">
    ...
    <div class="py-20 text-center">
        <div class="w-20 h-20 mx-auto mb-6 rounded-full bg-[#A8895F]/10 border border-[#A8895F]/30 flex items-center justify-center">
            <i data-lucide="package-open" class="w-10 h-10 text-[#A8895F]"></i>
        </div>
        <h3 class="font-serif font-bold text-2xl text-[#EDE5D8] mb-2">{{ __('Your fragrance journey is just beginning') }}</h3>
        ...
        <a href="{{ route('shop.index') }}"
           class="inline-block px-7 py-3 bg-[#A8895F] text-[#12100E] text-xs font-extrabold uppercase tracking-[0.3em] polygon-btn hover:bg-[#12100E] hover:text-[#F8F5EF] shadow-lg">
    ...
    <div class="space-y-5">
        ...
        <div class="border border-[#322B23] bg-[#17130F] polygon-card p-5 hover:shadow-md transition-shadow">
            <div class="flex flex-wrap items-start justify-between gap-4 mb-4 pb-4 border-b border-[#322B23]/60">
                <div>
                    <div class="flex items-center gap-3 flex-wrap">
                        <span class="font-mono font-extrabold text-sm tracking-wider text-[#EDE5D8]">{{ $order->order_number }}</span>
                        <span class="inline-block px-3 py-1 rounded text-[9px] uppercase tracking-[0.18em] font-extrabold {{ $order->statusBadgeClass }}">
                            {{ $order->statusLabel }}
                        </span>
                    </div>
                    <div class="flex flex-wrap items-center gap-4 mt-2 text-[11px] text-[#B5A897] font-bold">
                ...
                <div class="text-right">
                    <span class="text-[10px] font-extrabold text-[#A89C8C] uppercase tracking-[0.2em] block mb-1">{{ __('Order Total') }}</span>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 mb-4">
                ...
                <div class="flex flex-col gap-1.5 text-center">
                    <div class="aspect-square bg-[#0C0A09] polygon-card border border-[#322B23] overflow-hidden">
                    ...
                    <div class="min-h-0">
                        <p class="text-[10px] font-bold text-[#EDE5D8] line-clamp-2 leading-tight">{{ $item->name }}</p>
                ...
                <div class="aspect-square bg-[#221D19] text-[#F8F5EF] polygon-card border border-[#A8895F]/30 flex flex-col items-center justify-center gap-1">
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                <a href="{{ route('account.orders.show', $order->order_number) }}"
                   class="px-4 py-2 bg-[#A8895F] text-[#12100E] text-[10px] font-extrabold uppercase tracking-[0.2em] polygon-btn hover:bg-[#12100E] hover:text-[#F8F5EF] transition-all inline-flex items-center gap-2">
                ...
                <a href="{{ route('orders.track.submit') }}"
                   ...
                   class="px-4 py-2 bg-[#17130F] border border-[#A8895F]/50 text-[#EDE5D8] text-[10px] font-extrabold uppercase tracking-[0.2em] polygon-btn hover:bg-[#221D19] transition-all inline-flex items-center gap-2">
    ...
    @if($orders->hasPages())
    <div class="pt-6 mt-4 border-t border-[#322B23]/60">
        {{ $orders->links() }}
    </div>
    @endif
    @endif
