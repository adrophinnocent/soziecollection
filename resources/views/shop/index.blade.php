...
@section('title', 'Shop Perfumes | Sozie Collection')
...
<div class="bg-gradient-to-r from-[#F8F5EF] via-[#EDE5D8] to-[#F8F5EF] border-b border-[#D8C9B8] py-12 relative overflow-hidden transition-colors">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <span class="text-xs font-extrabold text-[#A8895F] uppercase tracking-[0.3em] block mb-1">SOZIE FRAGRANCE CATALOG</span>
        <h1 class="font-serif font-bold text-4xl sm:text-5xl text-[#29241F]">THE FULL COLLECTION</h1>
        <p class="text-xs sm:text-sm text-gray-700 mt-2 font-semibold">Explore handcrafted women's, men's, and unisex luxury perfumes, oils, and gift sets.</p>
    </div>
</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="space-y-8">
            <form action="{{ route('shop.index') }}" method="GET" class="glass-panel p-6 polygon-card border border-[#D8C9B8] space-y-6 bg-[#F8F5EF]">
                ...
                <!-- Search Input -->
                <div>
                    <label class="block text-xs font-extrabold text-[#A8895F] uppercase tracking-widest mb-2">Search</label>
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Fragrance name, notes..."
                ...
                <!-- Gender Filter -->
                <div>
                    <label class="block text-xs font-extrabold text-[#A8895F] uppercase tracking-widest mb-2">Gender</label>
                ...
                <!-- Category Filter -->
                <div>
                    <label class="block text-xs font-extrabold text-[#A8895F] uppercase tracking-widest mb-2">Category</label>
                ...
                <div>
                    <label class="block text-xs font-extrabold text-[#A8895F] uppercase tracking-widest mb-2">Scent Note</label>
                    <select name="scent_type" class="w-full bg-white border border-[#D8C9B8] text-xs text-[#29241F] px-3 py-2.5 focus:outline-none focus:border-[#A8895F] font-bold">
                        <option value="">All Scent Notes</option>
                ...
                <!-- Collections Filter -->
                <div>
                    <label class="block text-xs font-extrabold text-[#A8895F] uppercase tracking-widest mb-2">Collection</label>
                    <div class="space-y-2 text-xs text-[#29241F]">
                        <label class="flex items-center gap-2 cursor-pointer font-bold">
                ...
                <!-- Sort By -->
                <div>
                    <label class="block text-xs font-extrabold text-[#A8895F] uppercase tracking-widest mb-2">Sort By</label>
        ...
        <div class="lg:col-span-3 space-y-6">
            ...
            <div class="flex justify-between items-center text-xs text-[#29241F] font-bold">
                <span>Showing <strong class="text-[#A8895F] font-extrabold">{{ $products->total() }}</strong> perfumes</span>
            </div>
            ...
            <div class="glass-panel p-16 text-center polygon-card border border-[#D8C9B8] bg-[#F8F5EF]">
                ...
                <h3 class="font-serif font-bold text-2xl text-[#29241F]">No perfumes found</h3>
                <p class="text-xs text-gray-700 mt-2 font-medium">Try adjusting your filter preferences or search term.</p>
                ...
                <div class="navy-card p-4 polygon-card border border-[#D8C9B8] group hover:border-[#A8895F] transition-all duration-300 flex flex-col justify-between relative bg-[#F8F5EF]">
                    ...
                    <div>
                        ...
                        <span class="text-[10px] font-extrabold text-[#A8895F] uppercase tracking-widest block mb-1">
                            {{ $product->gender }} • {{ $product->category ? $product->category->name : 'Fragrance' }}
                        </span>
